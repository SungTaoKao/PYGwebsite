...
(function(){
	...
	function Datepickk(args){
		...
		function generateInputs(){
			...
			for(var x = 0;x<months;x++){
				var container = document.createElement('div');
					container.setAttribute('class','dpickk-table');
				for(var i = 0;i<42;i++){
		...
		function generateDates(year,month){
			var monthElements = that.el.querySelectorAll('.dpickk-table');
			var ws = (weekStart !== null) ? weekStart : languages[lang].weekStart;

			[].slice.call(that.el.querySelectorAll('.dpickk-table')).forEach(function(element, index) {
				...
				[].slice.call(element.querySelectorAll('.dpickk-table input')).forEach(function(inputEl,i) {
		...
		function resetCalendar(){
			[].slice.call(that.el.querySelectorAll('.dpickk-table input')).forEach(function(inputEl) {
				inputEl.checked = false;
			});
